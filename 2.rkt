...
; ********************************************************
; Name: Allan Ding 
; Email address: allan.ding@yale.edu heyooo
; ********************************************************
...
(define (bin2dec_helper lst total counter)
  (if (empty? lst)
      total
      (if (equal? (car lst) 0)
          (bin2dec_helper (cdr lst) (+ 0 total) (* 2 counter))
          (if (equal? (car lst) 1)
              (bin2dec_helper (cdr lst) (+ counter total) (* 2 counter))
              total))))
                          

(define (bin2dec num)
  (bin2dec_helper (reverse num) 0 1))


(define (find_start n square)
  (if (equal? 1 n)
      square
      (if (>= n square)
          (find_start (+ 0 n) (* 2 square))
          (quotient square 2))))

(define (make_list start n lst)
  (if (equal? 0 start)
      lst
      (if (>= n start)
          (make_list (quotient start 2) (- n start) (cons 1 lst))
          (if (>= start n)
              (make_list (quotient start 2) (+ 0 n) (cons 0 lst))
              lst))))

(define (dec2bin n)
  (let ([start (find_start n 1)]
        )
    (reverse(make_list start n '()))))
...
(define statealist '((A . 10) (B . 11) (C . 12) (D . 13) (E . 14) (F . 15) (a . 10) (b . 11) (c . 12) (d . 13) (e . 14) (f . 15)))

(define (helper_hex2dec lst exp total)
  (if (empty? lst)
      total
      (if (assq (car lst) statealist)
          (helper_hex2dec (cdr lst) (+ 1 exp) (+ total (* (cdr (assq (car lst) statealist)) (expt 16 exp))))
          (if (and (integer? (car lst)) (>= (car lst) 0) (<= (car lst) 9))
              (helper_hex2dec (cdr lst) (+ 1 exp) (+ total (* (car lst) (expt 16 exp))))
              #f)
           )))

(define (hex2dec num)
  (helper_hex2dec (reverse num) 0 0))



(define statealistd2h '((10 . A) (11 . B) (12 . C) (13 . D) (14 . E) (15 . F)))


(define (find_max_exp num max_exp)
  (if (>= num (expt 16 (+ 1 max_exp)))
      (find_max_exp num (+ 1 max_exp))
      max_exp))


(define (helper_dec2hex num lst max_exp)
  (if (and (equal? 0 num) (equal? -1 max_exp))
      lst
      (let ([div (quotient num (expt 16 max_exp))])
        (if (>= num (* (expt 16 max_exp) div)) ;; if number is greater than or equal to the divisor * base 16 number
            (if (>= div 10)
                (helper_dec2hex (- num (* (expt 16 max_exp) div)) (cons (cdr (assq div statealistd2h))lst) (- max_exp 1))
                (helper_dec2hex (- num (* (expt 16 max_exp) div)) (cons div lst) (- max_exp 1)))  ;; write if greater than 9 or less than or equal to 
            lst)
        )))

(define (dec2hex num)
  (reverse (helper_dec2hex num '() (find_max_exp num 0))))
...
(define (helper_my_remove rm lst newlst proc)
  (if (empty? lst)
      newlst
      (if (empty? rm)
          (helper_my_remove '() (cdr lst) (cons (car lst) newlst) proc)
          (if (proc rm (car lst))
                   (helper_my_remove '() (cdr lst) newlst proc) 
                   (helper_my_remove rm (cdr lst) (cons (car lst) newlst) proc)))))


(define (my-remove value lst [proc equal?])
  (reverse (helper_my_remove value lst '() proc)) ;; are we allowd to use reverse for my-remove 
  )

(define (helper-my-reverse lst newlst)
  (if (empty? lst)
      newlst
      (helper-my-reverse (cdr lst) (cons (car lst) newlst))
      ))

(define (my-reverse lst)
  (helper-my-reverse lst '()))
; ********************************************************
; ** problem 4 ** (10 points)
...
(define (sorted? lst . compare?)
  (if (or (empty? lst) (equal? (length lst) 1))
      #t
      (if (empty? compare?)
          (if (<= (car lst) (car (cdr lst)))
              (sorted? (cdr lst))
              #f)
          (if ((car compare?) (car lst) (car (cdr lst)))
...
(define (helper_inflate lst newlst)
  (if (empty? lst)
      newlst
      (if (integer? (car lst))
          (helper_inflate (cdr lst) (cons (+ 1 (car lst)) newlst))
          (helper_inflate (cdr lst) (cons (car lst) newlst)))))

(define (inflate lst)
  (reverse (helper_inflate lst '())))
...
(define (helper_iterate start proc n newlst)
  (if (equal? 0 n)
      newlst
      (helper_iterate (proc start) proc (- n 1) (cons (proc start) newlst))))

(define (iterate start proc n)
  (if (equal? 0 n)
      start
  (reverse (helper_iterate start proc n '()))))
...
(define (helper_compound start proc test newlst val)
  (if (equal? val 0)
      newlst
      (if (test (proc start))
          (helper_compound (proc start) proc test (cons (proc start) newlst) 0)
          (helper_compound (proc start) proc test (cons (proc start) newlst) 1))))
  
(define (compound start proc test)
  (reverse(helper_compound start proc test '() 1)))
...
(define (f ele lst)
  (if (empty? lst)
      '(())         
      (map (lambda (x) (cons ele x)) lst)))


(define (power-set xs)
  (if (empty? xs)
      (list empty)
      (append (power-set (cdr xs)) (f (car xs) (power-set (cdr xs))))))
...
(define (multiply_elements lst) 
  (if (empty? lst)
      1
      (* (car lst) (multiply_elements (cdr lst)))))

(define (simplify big_lst)
  (if (empty? big_lst)
      '()
      (map (lambda (x) (multiply_elements x)) big_lst)))


(define (all-factors n)
  (let ([base_primes (prime-factors n)])
    (sort (remove-duplicates (simplify (power-set base_primes))) <)))
